/* Name: Eric Grounds 
 * Date: 1/24/2013 
 * Source File: /usr/src/servers/is/dmp_kernel.c
 * Changes Made: Changed privaleges_dmp to messagecount_dmp
 *		 (Starting on line 343).   
 *  		 This function displays the number of messages  
 *		 sent between processes. Each process is displayed
 *		 one by one with a list of the processes it sent 
 *		 messages to along with the number of messages sent   
 */
 ...
 *				messagecount_dmp                             *				     *
 *===========================================================================*/
PUBLIC void messagecount_dmp()
{
  register struct proc *rp;
  static struct proc *oldrp = BEG_PROC_ADDR;
  int r, n = 0, i = 0, j = 0, columns = 0;
  ...
  /* Display the count of messages sent between each process */
  /* The message counts are displayed in 3 colummns */
  /* The names of the processes that received the message(s) are listed
  /* by name along with the count of messages that were received */ 
  /* If a process hasn't sent any messages, there will be no process */
  /* names or counts listed.  It will simply be blank. */ 
  ...
  for (rp = oldrp; rp < END_PROC_ADDR; ++rp) {
    ...
    for (i = 0; i < NR_TASKS + NR_PROCS; i++) {
      if (rp->messageCount[i] != 0) {
         ++columns;
       	 printf("%-8s %8d| ", proc_name(i), rp->messageCount[i]);
	 if (columns % 3 == 0) {
            printf("\n");
            columns = 0;
	 }
