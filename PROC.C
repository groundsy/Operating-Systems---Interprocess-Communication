/* Name: Eric Grounds
 * Date: 1-24-2013
 * Source File: /usr/src/kernel/proc.c
 * Changed Made: Added ++(caller_ptr->messageCount[dst_ptr->p_nr + NR_TASKS])
 *  		 to line 313. caller_ptr is the sender of the message and dst_prt
 * 		 is the receiver. This line will increment the sent messages count by 1 
 * 		 with respect to the caller and receiver. 
 */
 ...
 */
  ...
  if ( (RTS_ISSET(dst_ptr, RECEIVING) && !RTS_ISSET(dst_ptr, SENDING)) &&
	...
	/* Increment the message count by 1 each time a message is sent */ 
	++(caller_ptr->messageCount[dst_ptr->p_nr + NR_TASKS]);
